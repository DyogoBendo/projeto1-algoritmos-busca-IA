    ...
    while(!frontier.empty() and !found){        
        ...
        if(!found){
            for(auto [u, d] : g.node_edges[node.state]){
                Node prox(u, d, g.node_heuristic[u]);                
                if(!generated_nodes.count(u) or generated_nodes[u] > prox.f()){
                    frontier.erase(Node(u, generated_nodes[u], g.node_heuristic[u]));

                    generated_nodes[u] = prox.f();
